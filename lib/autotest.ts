...
// One full liveness sweep across every site's every key. Mirrors the manual
// /api/test/site sweep — balance is account-level so it's fetched ONCE per site
// and reused for that site's keys, keeping the per-invocation Cloudflare
// subrequest count down. Sites are walked SEQUENTIALLY (keys within a site in
// parallel, like the manual endpoint) so a multi-site account doesn't open a
// huge simultaneous connection burst. Uses the same minimum-token probe as
// manual 测活 (max_tokens:1 — only a genuine success spends ~2 tokens, every
// failure spends 0), so a daily run costs at most ~2 tokens per working key.
export async function runDailyAutoTest(env: Env): Promise<void> {
  ...
  for (const site of sites) {
    const keys =
      (await env.DB.prepare("SELECT * FROM api_keys WHERE site_id=?1").bind(site.id).all<ApiKey>()).results ?? [];
